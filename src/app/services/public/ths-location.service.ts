...
export class ThsLocationService {
  public locData = {
    'latitude': 39.908643,
      ...
      return new Promise(resolve => {
        navigator.baidulocation.get((message) => {
          ...
          if (lat !== 5e-324 && lng !== 5e-324) {
            ...
            this.locData.latitude = loc.lat;
            this.locData.longitude = loc.lon;
   ...
   */
  public startLoc(isStart: boolean, interval = 50000) {
    let intervalId;
  ...
  private _startLoc() {
    this.startLocation().then((pos) => {
      if (this.locationListener !== undefined && this.locationListener !== null) { // 设置位置监听回掉
        this.locationListener.updateLoc(pos);
      }
    }).catch();
  }
